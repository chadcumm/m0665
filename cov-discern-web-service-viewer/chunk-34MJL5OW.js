...
var JsonViewer = class _JsonViewer {
  ...
  /** Service name used for XLSX export filename */
  exportName = input("", ...ngDevMode ? [{ debugName: "exportName" }] : []);
  ...
  setMode(mode) {
    const label = mode === "tree" ? "Tree" : mode === "code" ? "Code" : "Table";
    const toastId = this.toast.show(`Loading ${label} view...`, "info", 0);
    ...
    if (mode === "tree" || mode === "code") {
      ...
      setTimeout(() => this.toast.dismiss(toastId), 300);
    }
  }
  ...
  /**
   * Export response data to XLSX with denormalized rows.
   *
   * For nested structures like Documents > Actions, parent fields repeat
   * on each child row (like a SQL join). A document with 3 actions produces
   * 3 rows — document columns repeat, action columns differ.
   *
   * Creates one worksheet per top-level array found in the response.
   */
  async exportToXlsx() {
    const d = this.data();
    if (!d)
      return;
    const toastId = this.toast.show("Preparing XLSX export...", "info", 0);
    try {
      ...
      if (arrays.length > 0) {
        for (const arr of arrays) {
          this.toast.update(toastId, `Processing ${arr.label} (${arr.data.length} rows)...`);
          const flatRows = this.denormalizeArray(arr.data);
        ...
        const flatRows = [this.extractScalars(d)];
        const ws = XLSX.utils.json_to_sheet(flatRows);
        XLSX.utils.book_append_sheet(wb, ws, "Response");
      }
      const filename = this.buildExportFilename();
      this.toast.update(toastId, "Writing file...");
      XLSX.writeFile(wb, filename);
      this.toast.dismiss(toastId);
      this.toast.show(`Exported ${filename}`, "success");
    } catch (err) {
      this.toast.dismiss(toastId);
      this.toast.show(`Export failed: ${err.message}`, "error", 5e3);
    }
  }
  /**
   * Denormalize an array of objects into flat rows.
   * Parent scalar fields repeat for each child array entry.
   *
   * Example: A document with 2 actions produces 2 rows where
   * document columns (EVENT_ID, PATIENT_NAME, etc.) repeat and
   * action columns (ACTION_TYPE, ACTION_STATUS, etc.) differ.
   */
  denormalizeArray(data, prefix = "") {
    const rows = [];
    for (const item of data) {
      ...
      const parentFields = this.extractScalars(item, prefix);
      const nestedArrays = Object.entries(item).filter(([, v]) => Array.isArray(v) && v.length > 0).map(([key, value]) => ({
        key: prefix ? `${prefix}.${key}` : key,
        data: value
      }));
      if (nestedArrays.length === 0) {
        rows.push(parentFields);
      } else {
        for (const nested of nestedArrays) {
          const childRows = this.denormalizeArray(nested.data, nested.key);
          for (const childRow of childRows) {
            rows.push(__spreadValues(__spreadValues({}, parentFields), childRow));
          }
        }
      }
    }
  ...
  /** Extract scalar fields from an object (skip arrays and nested objects) */
  extractScalars(obj, prefix = "") {
    const result = {};
    for (const [key, value] of Object.entries(obj || {})) {
      ...
      if (Array.isArray(value)) {
        ...
        Object.assign(result, this.extractScalars(value, fullKey));
      } else {
  ...
  /** Build export filename from service name + current date/time */
  buildExportFilename() {
    const name = this.exportName() || "response";
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(JsonViewer, { className: "JsonViewer", filePath: "src/app/components/json-viewer/json-viewer.ts", lineNumber: 36 });
})();
...
function Executor_Conditional_19_Conditional_2_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275element(0, "app-customized-dot", 27);
  }
}
function Executor_Conditional_19_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275text(1);
    \u0275\u0275conditionalCreate(2, Executor_Conditional_19_Conditional_2_Template, 1, 0, "app-customized-dot", 27);
    \u0275\u0275elementEnd();
  ...
  if (rf & 2) {
    ...
    const ctx_r1 = \u0275\u0275nextContext();
    \u0275\u0275advance();
    \u0275\u0275textInterpolate1(" ", svc_r4.description, " ");
    \u0275\u0275advance();
    \u0275\u0275conditional(ctx_r1.registry.isDefault(svc_r4.id) && ctx_r1.registry.hasUserVersion(svc_r4.id) ? 2 : -1);
    \u0275\u0275advance(5);
...
function Executor_Conditional_29_Template(rf, ctx) {
  if (rf & 1) {
    const _r5 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "label", 18)(1, "input", 30);
...
function Executor_Conditional_30_Template(rf, ctx) {
  if (rf & 1) {
    const _r6 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "div", 19)(1, "app-parameter-form", 31);
...
function Executor_Conditional_33_Conditional_19_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275element(1, "app-json-viewer", 44);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const result_r8 = \u0275\u0275nextContext();
    \u0275\u0275advance();
    \u0275\u0275property("data", result_r8.response)("exportName", result_r8.serviceName);
  }
}
...
function Executor_Conditional_33_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275elementStart(0, "div", 33)(1, "div", 34)(2, "h2");
    \u0275\u0275text(3);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(4, "div", 35)(5, "span", 36);
    \u0275\u0275text(6);
    \u0275\u0275elementEnd();
    \u0275\u0275conditionalCreate(7, Executor_Conditional_33_Conditional_7_Template, 2, 1, "span", 37);
    ...
    \u0275\u0275text(12);
    \u0275\u0275elementEnd()()();
    \u0275\u0275elementStart(13, "div", 40)(14, "code", 41);
    ...
    \u0275\u0275text(18, "close");
    \u0275\u0275elementEnd()()();
    \u0275\u0275conditionalCreate(19, Executor_Conditional_33_Conditional_19_Template, 2, 2, "div", 42);
    \u0275\u0275conditionalCreate(20, Executor_Conditional_33_Conditional_20_Template, 5, 1, "div", 43);
  }
  if (rf & 2) {
    const result_r8 = ctx;
    const ctx_r1 = \u0275\u0275nextContext();
...
function Executor_Conditional_35_For_15_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275text(5);
    \u0275\u0275conditionalCreate(6, Executor_Conditional_35_For_15_Conditional_6_Template, 2, 1, "span", 56);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(7, "code", 57);
    \u0275\u0275text(8);
    \u0275\u0275elementEnd()()();
    \u0275\u0275elementStart(9, "div", 58)(10, "span", 59);
    \u0275\u0275text(11);
    \u0275\u0275elementEnd();
...
function Executor_Conditional_35_Template(rf, ctx) {
  if (rf & 1) {
    ...
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(1, "div", 46)(2, "div", 47)(3, "h3");
    \u0275\u0275text(4, "Execution History");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(5, "div", 48)(6, "button", 24);
    ...
    \u0275\u0275repeaterCreate(14, Executor_Conditional_35_For_15_Template, 14, 15, "div", 50, _forTrack03);
    \u0275\u0275elementEnd()();
  }
  if (rf & 2) {
    const ctx_r1 = \u0275\u0275nextContext();
...
function Executor_Conditional_36_Template(rf, ctx) {
  if (rf & 1) {
    const _r12 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "app-help-modal", 61);
...
var Executor = class _Executor {
  ...
  async onExecute(parameterValues) {
    ...
    if (!service)
      return;
    const toastId = this.toast.show(`Executing ${service.displayName}...`, "info", 0);
    let result;
    ...
    this.toast.dismiss(toastId);
    if (result.success) {
      this.toast.show(`${service.displayName} completed in ${this.formatDuration(result.durationMs)}`, "success");
    } else {
      this.toast.show(`${service.displayName} failed: ${result.errorMessage}`, "error", 5e3);
    }
    const loadingId = this.toast.show("Loading response into viewer...", "info", 0);
    this.currentResult.set(result);
    setTimeout(() => this.toast.dismiss(loadingId), 500);
  }
...
${JSON.stringify(ref, null, 2)}`);
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _Executor, selectors: [["app-executor"]], decls: 37, vars: 18, consts: [[1, "executor-layout"], [1, "left-panel"], [1, "left-panel-scroll"], [1, "service-selector"], [1, "service-selector-header"], [1, "service-selector-actions"], ["title", "Help", 1, "btn", "btn-ghost", "btn-small", 3, "click"], [1, "material-icons"], [1, "btn", "btn-ghost", "btn-small", 3, "click", "disabled"], [1, "btn", "btn-ghost", "btn-small"], [3, "change", "value"], [3, "value"], [1, "mode-toggle-section"], [1, "mode-label"], [1, "mode-toggle"], [1, "btn", "btn-small", 3, "click"], [1, "btn", "btn-small", 3, "click", "disabled", "title"], [1, "co-init-spinner"], [1, "reference-checkbox"], [1, "parameter-section"], [1, "status-message", "loading"], [1, "right-panel"], [1, "empty-state"], ["title", "Executor Help"], [1, "btn", "btn-ghost", "btn-small", 3, "click"], [1, "history-count"], [1, "service-description"], ["tooltip", "This service has been customized from the default"], [1, "program-name"], [1, "label"], ["type", "checkbox", 3, "change", "checked"], [3, "executeRequested", "service", "executing"], [1, "spinner"], [1, "result-header"], [1, "result-info"], [1, "result-meta"], [1, "result-status"], [1, "result-mode"], [1, "result-duration"], [1, "btn", "btn-ghost", "btn-small", 3, "click", "title"], [1, "result-params-row"], [1, "result-params", 3, "click"], [1, "result-viewer"], [1, "error-panel"], [3, "data", "exportName"], [1, "history-overlay", 3, "click"], [1, "history-drawer"], [1, "drawer-header"], [1, "drawer-actions"], [1, "drawer-body"], [1, "history-item", 3, "success", "error", "active"], [1, "history-item", 3, "click"], [1, "history-left"], [1, "history-status-dot"], [1, "history-info"], [1, "history-name"], [1, "history-mode-badge"], [1, "history-params"], [1, "history-right"], [1, "history-time"], [1, "history-duration"], ["title", "Executor Help", 3, "closeRequested"], ["src", "https://mermaid.", "alt", "Executor process flow diagram", 2, "max-width", "100%", "height", "auto", "margin-bottom", "15px"]], template: function Executor_Template(rf, ctx) {
    if (rf & 1) {
      ...
      \u0275\u0275elementEnd();
      \u0275\u0275conditionalCreate(19, Executor_Conditional_19_Template, 8, 3);
      \u0275\u0275elementEnd();
        ...
        @if (selectedService(); as svc) {
          <p class="service-description">
            ...
            @if (registry.isDefault(svc.id) && registry.hasUserVersion(svc.id)) {
              <app-customized-dot tooltip="This service has been customized from the default" />
            }
          </p>
    ...
    @if (currentResult(); as result) {
      ...
      @if (result.success && result.response) {
        <div class="result-viewer">
          <app-json-viewer [data]="result.response" [exportName]="result.serviceName" />
        </div>
      }
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(Executor, { className: "Executor", filePath: "src/app/pages/executor/executor.ts", lineNumber: 21 });
})();
export {
  Executor
};
//# sourceMappingURL=chunk-34MJL5OW.js.map
